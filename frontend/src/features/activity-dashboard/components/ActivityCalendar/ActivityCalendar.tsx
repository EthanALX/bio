                ...
                <div key={weekIdx} className={styles.week}>
                  ...
                  <div className={styles.days}>
                    {week.days.map((dayData, dayIdx) => (
                      <div
                        key={dayIdx}
                        className={`${styles.day} ${!dayData ? styles.empty : styles[`level-${dayData.level}`]}`}
                        title={
                          dayData?.hasActivity && dayData?.distance
                            ? `${dayData.distance}km on ${dayData.date}`
                            : dayData?.hasActivity
                            ? `Activity on ${dayData.date}`
                            : ""
                        }
                      />
                    ))}
